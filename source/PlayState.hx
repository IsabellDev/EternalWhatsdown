...
class PlayState extends MusicBeatState
{
	...
	public var bombaiDoes:Bool = false;
	public var stageBack:BGSprite;
	...
	var thecamerasexymover:Bool = false;
	...
	override public function create()
	{
		...
		{
			case 'stage': //Week 1
				stageBack = new BGSprite('stageback', -600, -200, 0.9, 0.9);
				add(stageBack);
		...
		timeTxt.setFormat(Paths.font("pricedown.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("pricedown.otf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("pricedown.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function moveCameraSection():Void {
		...
		if (!SONG.notes[curSection].mustHitSection)
		{
			...
			thecamerasexymover = true;
		}
		else
		{
			moveCamera(false);
			callOnLuas('onMoveCamera', ['boyfriend']);
			thecamerasexymover = false;
		}
	}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'whatsnerd']);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
            ...
            if (!thecamerasexymover) {
                switch (note.noteData) {
                   case 0:
                        camFollow.set(boyfriend.getMidpoint().x - 100 + boyfriend.cameraPosition[0] - 40, boyfriend.getMidpoint().y - 100 + boyfriend.cameraPosition[1]);
                   case 1:
                        camFollow.set(boyfriend.getMidpoint().x - 100 + boyfriend.cameraPosition[0], boyfriend.getMidpoint().y - 100 + boyfriend.cameraPosition[1] + 40);
                   case 2:
                        camFollow.set(boyfriend.getMidpoint().x - 100 + boyfriend.cameraPosition[0], boyfriend.getMidpoint().y - 100 + boyfriend.cameraPosition[1] - 40);
                   case 3:
                        camFollow.set(boyfriend.getMidpoint().x - 100 + boyfriend.cameraPosition[0] + 40, boyfriend.getMidpoint().y - 100 + boyfriend.cameraPosition[1]);
				...
				case 'whats-destroyer':
					...
					{	
						case 256 | 1264 | 1776:
							...
							defaultCamZoom = 1;
						case 512 | 1280:
						    FlxTween.tween(FlxG.camera, {zoom: 0.8}, 0.5);
							defaultCamZoom = 0.8;
	...
	override function beatHit()
	{
		...
		if (curBeat % 4 == 0 && bombaiDoes) {
			curBeat % 4 == 0 ? {
				camGame.angle = 5;
				FlxTween.tween(camGame, {angle: 0}, 0.15, {ease: FlxEase.quadOut});
			} : {
				camGame.angle = -5;
				FlxTween.tween(camGame, {angle: 0}, 0.15, {ease: FlxEase.quadOut});
			}		
        ...
        //health icon bounce but i did your mom XD
		if (curBeat % gfSpeed == 0) {
			curBeat % (gfSpeed * 2) == 0 ? {
				iconP1.scale.set(0.8, 1.4);
				iconP2.scale.set(1.1, 0.8);
			} : {
				iconP1.scale.set(1.1, 0.8);
				iconP2.scale.set(0.8, 1.4);
			}
